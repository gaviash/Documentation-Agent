...
def _normalize_line_range(start_line, end_line, total_lines):
    ...
    try:
        ...
        end = total_lines if end_line is None else int(end_line)
    except (TypeError, ValueError):
        raise ValueError("start_line and end_line must be integers when provided.")
...
def read_file(
    ...
    try:
        ...
        line_range = _normalize_line_range(start_line, end_line, total_lines)

        if line_range is None:
            content = full_content
            returned_start_line = 1 if total_lines else None
            returned_end_line = total_lines if total_lines else None
        else:
            start, end = line_range
            content = "".join(lines[start - 1:end])
            returned_start_line = start if start <= total_lines else None
            returned_end_line = end if start <= total_lines else None
...
def web_search(query : str,search_depth : Literal['basic','advanced','fast','ultra-fast'] ='basic',max_results=5,timerange =None,chunks_per_source=3):
    ...
    - max_results : The maximum number of search results to return.Required range: 0 <= x <= 20.By default,set to 5.\n
    ...
    try:
        ...
        return _tool_result(True, response=response)
    except Exception as e:
        return _tool_result(False, str(e), response=None)
...
def web_fetch(url : str,query=None,chunks_per_source=3,extract_depth : Literal['basic','advanced'] ="basic"):
    ...
    Returns:
    A dictionary with success/error and response. The response contains the
    extracted content, raw_content, and extraction metadata returned by the API.
    """
    try:
        kwargs = {
            "urls": url,
        ...
        response = _get_tavily_client().extract(**kwargs)
        return _tool_result(True, response=response)
    except Exception as e:
        return _tool_result(False, str(e), response=None)
...
def shell(
    ...
    - cwd: Workspace directory where the command was executed.
    - timeout_seconds: Effective timeout after applying the hard cap.

    If the command times out, the tool returns success=False, return_code=None,
    and includes the timeout details in stderr.
    """
    try :
        timeout = _normalize_limit(
            timeout_seconds,
